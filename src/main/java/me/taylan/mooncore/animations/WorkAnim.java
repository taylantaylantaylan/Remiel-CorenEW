		...
		if (JoinListener.getMenu().get(p.getUniqueId()).firstEmpty() != -1) {
			...
			if (timer > 0) {
				p.sendMessage(Painter.paint(
						"&cZaten bir ekipman işliyorsun! 2. Bir ekipmanı işlemeye koymak için 10. seviye ve üzeri El Sanatları ustalığı gerekir."));
			}
			if (timer <= 0) {
			...
			p.sendMessage(Painter.paint("&c&lBir malzeme üretebilmek için &6&lEl Sanatları Deponu &c&lBoşalt!"));
		}
	}
}
